...
case $os_version in
  ...
  *10.6*)  cmdline_version="DeveloperToolsCLILeo"
           echo "Outdated OS. Considering upgrading before continuing.";; # Snow Leopard
...
if [ ! -z $(pkgutil --pkgs=com.apple.pkg.$cmdline_version) ]; then
  ...
  echo "Downloading and installing the GCC compiler."
  echo "When you're done rerun this script"
